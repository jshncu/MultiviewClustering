function [NMI, Acc, Purity] = myClusteringMeasure(true_labels,results)
...
% for i = 1 : times_clustering
	% [results, obj] = myKmeansClustering(G, cluster_num, init_labels(:, i));  % 把锚图Z的转置U做kmeans得到聚类结果

	% NMI(i) = nmi(true_labels,results);
	% conf_mat = confusionmat(true_labels,results); %matlab自带的函敿
	% purity = sum(max(conf_mat',[],2)) / length(true_labels);
	% Purity(i) = purity; %%纯度：每丿¸ª簇中朿¤的数据占总数据的比例
...
% 	[tmplabel, tmpobj] = myKmeansClustering(G, cluster_num, init_labels(:, i));  % 把锚图Z的转置U做kmeans得到聚类结果
% 	if obj > tmpobj
% 		results = tmplabel;
% 	end
% end
...
conf_mat = confusionmat(true_labels,results); %matlab自带的函敿
purity = sum(max(conf_mat',[],2)) / length(true_labels);
Purity = purity; %%纯度：每丿¸ª簇中朿¤的数据占总数据的比例
